...
These domains may consist of multiple domains within a profile - such as prevalence or targets.
...
In addition, indicators can vary by different area types: commissioners of primary care (known as Clinical Commissioning Groups or CCGs), hospital groups (acute trusts), administrative regions or general practitioner practices.
These geographies themselves fall within nested geographies, so CCGs may aggregate up to sub-region, region or nation depending on the base geographies. 

All indicators have a fundamentally identical structure; a code uniquely identifies a geography, three time variables reflect specify the year, quarter and month, variables for count and denominator provide the count and some denominator supplied where appropriate, a value column gives the actual value - such as a prevalence or rate - plotted in the various representations on the website. 
...
head(ftips_indicators)
```

The researcher may then wish to pull down the data for one or more indicators from one or more domains or profiles.
However, before they can do so, they need to check what geographies are represented by the indicators.

```{r}
# can't do this at the moment, see issue #16: https://gitlab.phe.gov.uk/sebastian.fox/fingertips/issues/16
```
...
To expand on the functions described above, supporse a researcher wishes to examine the relationship between socio-economic deprivation and life-expectancy at birth.
...
First, the research loads in the deprivation data at the level of the upper-tier local authorities: [ref parliamentary briefing paper on LA strucutre?]
...
The researcher can then limit the previously loaded data set giving life expectancy at birth to the level of County and Unitary Authority and merge the deprivation data on.
