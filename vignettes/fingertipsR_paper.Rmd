...
The web interface provides an interactive data visualisations allowing users to examine trends over time, geographical distribution and make comparisons between health providers. 
...
In addition, indicators can vary by different area types within two broad main geography themes, administrative and health: commissioners of health care services for local areas (known as Clinical Commissioning Groups or CCGs), hospital groups (acute trusts), upper- and lower-tier local authorities, administrative regions or general practitioner practices among others.
...
The API was created following open data principles so that the data on the Fingertips site could be available for anyone to access, use or share.[@ahmed_2012] <!--ST - Question for DF, is this an appropriate reference?-->
...
The data are transferred in JavaScript Object Notification (JSON), a lightweight data format that is easy for both humans to read and for machines to generate and parse.
...
ftips_indicators %>% 
        mutate(IndicatorName = str_trunc(as.character(IndicatorName), 
                                         width = 20, "right")) %>% 
        head()
```
...
As a result there is a lot of scope for extension to data available from the R package. 
...
GitHub provides an issue tracker in which users of softare can report problems with the software or suggest new features. 
...
At the date of writing, the package itself has been downloaded *n* times, *n* issues have been raised and *n* researchers have controbuted to the project repository. 

# Discussion/conclusions

The work presented here illustrate a number of new and important concepts in public health research. 
...
Open source software (softare where the code can be examined and adapted) allows the rapid development of new functionality, greatly expanding the uses to which it can be put and rapidly providing solutions to otherwise unmet need. 
